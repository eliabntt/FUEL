...
// SECTION kinodynamic replanning
		...
		bool FastPlannerManager::kinodynamicReplan(const Eigen::Vector3d &start_pt,
			...
			for(auto cca = 0; cca < 10; cca ++)
			{
				ROS_INFO_STREAM(start_pt << " " << start_vel << " " << start_acc << " " << end_pt << " " << end_vel);
			}
...
// !SECTION
		...
		bool FastPlannerManager::localExplore(Eigen::Vector3d start, Eigen::Vector3d start_vel,
			...
			if (dist_to_goal < 5.0) {
				...
				int cnt = 0;
				while (points.size() < sample_num && cnt < tries) {
					Eigen::Vector3d pt;
				...
				for (int acca = 0; acca < 100; acca ++)
					ROS_ERROR_STREAM("SNO QUI 4");
			...
			NonUniformBspline init(ctrl_pts, pp_.bspline_degree_, dt);

			vector<Eigen::Vector3d> start_bsp, end_bsp;
			init.getBoundaryStates(2, 0, start_bsp, end_bsp);
			bspline_optimizers_[0]->setBoundaryStates(start_bsp, end_bsp);
			int cost_func = BsplineOptimizer::NORMAL_PHASE;
			bspline_optimizers_[0]->optimize(ctrl_pts, dt, cost_func, 1, 1);
			...
			for (int i = 0; i < 3; ++i) {
				...
				traj.getBoundaryStates(2, 0, start_bsp, end_bsp);
				bspline_optimizers_[0]->setBoundaryStates(start_bsp, end_bsp);
